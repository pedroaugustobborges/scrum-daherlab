...
async function openChannel(externalId) {
  ...
  if (!data.ok)
    throw new Error(
      `conversations.open failed for ${externalId}: ${JSON.stringify(data)}`,
    );
...
async function postMessage(channelId, text) {
  ...
  if (!data.ok)
    throw new Error(
      `chat.postMessage failed for channel ${channelId}: ${JSON.stringify(data)}`,
    );
  return data.ts;
}
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!activeProjects || activeProjects.length === 0) {
      return res
        .status(200)
        .json({ message: "No active projects found", sent: 0 });
    }
    ...
    const staleProjects = activeProjects
      .map((p) => {
        ...
        const idleDays =
          maxTs !== null ? Math.floor((nowMs - maxTs) / 86_400_000) : null; // null = no tasks at all
        return { ...p, idleDays };
      })
