  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const teamsToAdd = selectedTeams.filter(
        (id) => !initialTeams.includes(id),
      );
      const teamsToRemove = initialTeams.filter(
        (id) => !selectedTeams.includes(id),
      );
      ...
      if (teamsToRemove.length > 0) {
        ...
        if (removeError) console.error("Error removing teams:", removeError);
      }
      ...
      if (teamsToAdd.length > 0) {
        ...
        if (addError) console.error("Error adding teams:", addError);
      }

      // Ada notifications — fire-and-forget after UI closes
      if (previousStatus !== "on-hold" && formData.status === "on-hold") {
        notifyProjectOnHold({
          projectId: project.id,
          projectName: formData.name,
          reason: formData.on_hold_reason.trim(),
        });
      } else if (previousStatus === "on-hold" && formData.status === "active") {
        notifyProjectReactivated({
          projectId: project.id,
          projectName: formData.name,
        });
            ...
            <Box sx={{ display: "flex", gap: 1.5 }}>
              {[
                ...
                },
                {
                  ...
                  bgActive: "rgba(107,114,128,0.12)",
