...
type FlatTaskItem = HierarchicalTask & { _depth: number; _visible: boolean; _hasChildren: boolean }
...
function getTaskDescendants(taskId: string, allTasks: HierarchicalTask[]): HierarchicalTask[] {
    ...
    <Dialog open={!!state} onClose={onClose} maxWidth="xs" fullWidth>
      ...
      <DialogContent>
        <DialogContentText sx={{ fontSize: '0.875rem' }}>
          ...
          <strong>{childrenCount} subtarefa{childrenCount > 1 ? 's' : ''}</strong>. Ao mover para{' '}
          <strong>{targetSprintName}</strong>, todas as subtarefas serão movidas também.
        </DialogContentText>
      </DialogContent>
...
function DroppableSprintHeaderRow({
  ...
  const { setNodeRef, isOver } = useDroppable({ id: `sprint-header-${sprint.id}` })
  ...
  const startFmt = !isBacklog ? formatSprintDate((sprint as ProjectSprint).start_date) : null
  const endFmt = !isBacklog ? formatSprintDate((sprint as ProjectSprint).end_date) : null
    ...
    >
      ...
      <TableCell colSpan={11} sx={{ pl: '10px !important' }}>
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5 }}>
          ...
          <Tooltip title={isBacklog ? 'Adicionar ao backlog' : `Adicionar tarefa em ${sprint.name}`}>
            <IconButton
              size="small"
          ...
          <Box sx={{ display: 'flex', alignItems: 'center', pl: task._depth * 3 }}>
            {task._hasChildren ? (
              <IconButton
                ...
                sx={{ mr: 0.5 }}
              >
                {expandedIds.has(task.id) ? <ExpandMore fontSize="small" /> : <ChevronRight fontSize="small" />}
              </IconButton>
            ) : (
              <Box sx={{ width: 28 }} />
            )}
            ...
            {statusOptions.map((opt) => (
              <MenuItem key={opt.value} value={opt.value}>
                <Chip
                  size="small"
                  label={opt.label}
                  sx={{ bgcolor: alpha(opt.color, 0.1), color: opt.color, fontWeight: 600, height: 24 }}
                />
              </MenuItem>
            ))}
          </Select>
      ...
      case 'end_date': {
        ...
        return isEditing ? (
          ...
          <Typography
            ...
            onClick={() => onStartEdit(task.id, field, dateValue ? dateValue.split('T')[0] : '')}
          >
      ...
      case 'planned_duration': {
        ...
        return isEditing ? (
          ...
          <Typography
            variant="body2"
            sx={{ cursor: 'pointer', '&:hover': { bgcolor: 'rgba(99, 102, 241, 0.05)' } }}
            onClick={() => onStartEdit(task.id, 'planned_duration', String(dur || ''))}
          >
            {dur ? `${dur}d` : '-'}
          </Typography>
        )
        ...
        return (
          <Select
            ...
            renderValue={(sel) => {
              ...
              const member = teamMembers.find((m) => m.id === sel) || assignedProfile
              return (
                <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                  <Avatar src={member?.avatar_url || undefined} sx={{ width: 24, height: 24, fontSize: 12 }}>
                    {member?.full_name?.charAt(0)?.toUpperCase() || '?'}
                  </Avatar>
                  <Typography variant="body2" noWrap>{member?.full_name || 'Usuário'}</Typography>
                </Box>
              )
            }}
          >
    ...
    <TableRow
      ...
      sx={{
        ...
        bgcolor: isDropTarget
          ? isDarkMode ? alpha('#6366f1', 0.15) : alpha('#6366f1', 0.07)
          : task.is_summary ? alpha('#6366f1', 0.03) : 'inherit',
      ...
      <TableCell>
        <Box sx={{ display: 'flex', gap: 0.5 }}>
          ...
          <Tooltip title="Excluir">
            <IconButton size="small" onClick={() => onDelete(task.id)} sx={{ color: 'error.main' }}>
...
export default function ProjectGridView({ projectId }: ProjectGridViewProps) {
  ...
  useEffect(() => {
    const fetchTeamMembers = async () => {
      ...
      if (!projectTeams?.length) { setTeamMembers([]); return }

      const teamIds = projectTeams.map((pt) => pt.team_id)
      const { data: members } = await supabase
        .from('team_members')
        .select('user_id, profiles:profiles!team_members_user_id_fkey(id, full_name, avatar_url)')
        .in('team_id', teamIds)

      const unique = new Map<string, TeamMember>()
      members?.forEach((m) => {
        const p = m.profiles as unknown as TeamMember
        if (p && !unique.has(p.id)) unique.set(p.id, { id: p.id, full_name: p.full_name, avatar_url: p.avatar_url })
      })
      setTeamMembers(Array.from(unique.values()).sort((a, b) => a.full_name.localeCompare(b.full_name)))
    }
  ...
  const groupedRenderList = useMemo((): RenderItem[] => {
    ...
    if (backlogRoots.length > 0) {
      items.push({ _type: 'sprint-header', sprint: { id: 'backlog', name: 'Backlog', status: 'backlog' } })
  ...
  const visibleTaskItems = useMemo(
    () => groupedRenderList.filter((item): item is FlatTaskItem => !('_type' in item)),
    [groupedRenderList]
  )

  const sortableIds = useMemo(() => visibleTaskItems.map((t) => t.id), [visibleTaskItems])
  ...
  const handleSaveEdit = useCallback(async () => {
    ...
    if (['percent_complete', 'planned_duration', 'story_points'].includes(field)) {
      updateValue = parseInt(editValue) || 0
    }
    if (field === 'start_date' && editValue) {
      if (task.end_date) extra.planned_duration = calcDuration(editValue, task.end_date)
      else if (task.planned_duration) extra.end_date = calcEndDate(editValue, task.planned_duration)
    }
    ...
    if (field === 'planned_duration' && updateValue && task.start_date) {
      extra.end_date = calcEndDate(task.start_date.split('T')[0], updateValue as number)
    }

    await updateTask.mutateAsync({ id, projectId, updates: { [field]: updateValue, ...extra } })
    setEditingCell(null)
    setEditValue('')
  }, [editingCell, editValue, tasks, updateTask, projectId])
  ...
  // ── Task CRUD ─────────────────────────────────────────────────────────────────
  const handleAddTask = useCallback(async (parentId?: string, sprintId?: string | null) => {
    const parent = parentId ? tasks.find((t) => t.id === parentId) : null
    const resolvedSprintId = sprintId !== undefined ? sprintId : (parent?.sprint_id ?? null)
  ...
  const handleDeleteSelected = useCallback(async () => {
    ...
    if (!window.confirm(`Tem certeza que deseja excluir ${selectedIds.size} tarefa(s)?`)) return
    for (const id of selectedIds) await deleteTask.mutateAsync({ id, projectId })
  ...
  const handleIndent = useCallback(async () => {
    for (const id of selectedIds) {
      const task = tasks.find((t) => t.id === id)
      if (task) try { await indentTask.mutateAsync({ task, tasks }) } catch { /* handled */ }
    }
  ...
  const handleOutdent = useCallback(async () => {
    for (const id of selectedIds) {
      const task = tasks.find((t) => t.id === id)
      if (task) try { await outdentTask.mutateAsync({ task, tasks }) } catch { /* handled */ }
    }
  }, [selectedIds, tasks, outdentTask])

  // ── Status / blocking ────────────────────────────────────────────────────────
  const handleStatusChange = useCallback((task: HierarchicalTask, newStatus: TaskStatus) => {
    ...
    if (task.status === 'blocked' && newStatus !== 'blocked') updates.blocked_comment_id = null
  ...
  const handleBlockConfirm = useCallback(async (reason: string) => {
    ...
    try {
      const { data: commentData, error } = await supabase
        .from('comments')
        .insert({ task_id: pendingBlockTask.id, user_id: user.id, content: `🚫 **Motivo do Bloqueio:** ${reason}` })
        .select().single()
      if (error) throw error
      await updateTask.mutateAsync({ id: pendingBlockTask.id, projectId, updates: { status: 'blocked' as TaskStatus, blocked_comment_id: commentData.id } })
      toast.success('Tarefa bloqueada')
    } catch { toast.error('Erro ao bloquear tarefa') }
  ...
  const executeSprintMove = useCallback(async (
    task: HierarchicalTask,
    targetSprintId: string | null,
    descendants: HierarchicalTask[],
  ) => {
    try {
      await updateTask.mutateAsync({ id: task.id, projectId, updates: { sprint_id: targetSprintId } })

      if (descendants.length > 0) {
        await Promise.all(
          descendants.map((d) =>
            supabase.from('tasks').update({ sprint_id: targetSprintId }).eq('id', d.id)
          )
        )
  ...
  const executeReorder = useCallback(async (
    draggedTask: HierarchicalTask,
    targetTask: HierarchicalTask,
  ) => {
    // Scope = all tasks at the same hierarchy level
    const scope = tasks
      .filter((t) => t.parent_task_id === (draggedTask.parent_task_id ?? null))
    ...
    const updates = reordered.map((t, i) => ({ id: t.id, order_index: i + 1 }))

    // Cross-sprint detection (only for root tasks)
    const isCrossSprint = !draggedTask.parent_task_id && draggedTask.sprint_id !== targetTask.sprint_id
    ...
    if (overId.startsWith('sprint-header-')) {
      ...
      if (descendants.length > 0) {
        ...
        setConfirmMove({
          ...
          childrenCount: descendants.length,
          targetSprintName: sprintName,
          onConfirm: () => executeSprintMove(draggedTask, targetSprintId, descendants),
        })
      } else {
  ...
  const taskHandlers: TaskHandlers = useMemo(() => ({
    ...
    projectId,
    isDarkMode,
  }), [editingCell, editValue, handleStartEdit, handleSaveEdit, handleKeyDown, handleStatusChange, updateTask, projectId, teamMembers, expandedIds, toggleExpand, isDarkMode])
  ...
  return (
    <Box sx={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
      <GridToolbar
        ...
        onAddTask={() => handleAddTask(undefined, null)}
        onAddSubtask={() => {
      ...
      >
            ...
            <Table stickyHeader size="small">
              <TableHead>
                <TableRow>
                  ...
                  >
                    <Checkbox
                      indeterminate={selectedIds.size > 0 && selectedIds.size < visibleTaskItems.length}
                      checked={visibleTaskItems.length > 0 && selectedIds.size === visibleTaskItems.length}
                      onChange={selectAll}
                    />
                  </TableCell>
                  {[
        ...
        <DragOverlay dropAnimation={null}>
          {activeDragTask && (
            ...
            >
              <DragIndicator sx={{ color: '#6366f1', fontSize: 18, flexShrink: 0 }} />
              ...
              {activeDragTask.is_summary && (
                <Chip
                  label={`+filhos`}
                  size="small"
                  sx={{ height: 18, fontSize: '0.65rem', bgcolor: alpha('#6366f1', 0.1), color: '#6366f1', ml: 0.5 }}
                />
              )}
            </Box>
          )}
      ...
      <BlockReasonModal
        ...
        onClose={() => { setBlockReasonModalOpen(false); setPendingBlockTask(null) }}
